...
| Buttons ×3 | 10 kΩ pull-up to 3V3; **100 nF from the GPIO pin to GND**; **1 kΩ in series between that pin and the switch**; switch's other leg to GND | recommended. **Placement is not interchangeable** (this line was ambiguous before 2026-08-15 and is now fixed): the cap must sit ON the pin so it shunts a transient arriving at the pin, and the 1 kΩ must sit between the charged cap and the closing contact so it is the only discharge path — limiting it to 3.3 mA instead of the several amps that wiring resistance alone allows, in the same ground the mic returns on. Fit BOTH or NEITHER; a 100 nF with no series R is directly across the contacts. Arithmetic: fall τ = 1 kΩ×100 nF = 100 µs; rise τ = 10 kΩ×100 nF = 1 ms (4.5 ms on the internal ~45 kΩ pull-up alone) — both far under the firmware's 30 ms debounce, so the RC can neither mask a real press nor invent one. V_low = 3.3×1k/11k = 300 mV vs VIL_max 825 mV |
