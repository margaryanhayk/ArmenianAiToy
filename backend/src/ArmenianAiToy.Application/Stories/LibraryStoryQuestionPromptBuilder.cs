...
internal static class LibraryStoryQuestionPromptBuilder
{
    ...
    public const string AnswerRules =
        ...
        - (A) SINGLE-WORD MEANING question ("... ի՞նչ է", "... ո՞րն է", "ի՞նչ է նշանակում" about ONE word): answer with ONLY a simple, correct, child-friendly definition of that word — ONE short sentence is best, like a kind grown-up explaining to a small child. Do NOT pad with a second sentence; a second sentence is allowed ONLY if it is also a plain, correct, common-word fact about the word itself. Do NOT add any story-plot details, do NOT say "in our story/tale", do NOT mention what any character did with it. JUST explain the word. This is the one case you answer from real-world meaning, not from the story. Every word you use must be a real, common Armenian word; if you are genuinely unsure of the word's meaning, say so gently and NEVER invent or misuse a word.
        - (A2) SET-PHRASE question (the child asks about a PHRASE of two or more words — an oath, a saying, a blessing, an idiom; e.g. «հորս կնունքով», «մորս ծնունդով», «հորս արևով», «աչքիս լույս»): Armenian is full of these, and their meaning is NOT the sum of the separate words. Explain what a person MEANS when they say it, in ONE short sentence a small child understands, and make clear the words are not meant exactly as they sound. NEVER answer by defining the individual words instead — explaining «կնունք» when the child asked about «հորս կնունքով» is a WRONG answer. If the phrase is a way of swearing that something is true, say simply that the speaker is promising he is telling the truth. You may add at most ONE short clause about when people say it. Decide (A2) before (A): if the child quoted more than one word, it is (A2).
        ...
        - For a STORY (B) answer, end on a concrete, calm statement about what actually happened in the story. Do NOT end with a vague, philosophical, summarizing, or meta sentence (e.g. "anything seems possible", "it is an ironic and funny story", "it creates funny moments"). For a WORD-MEANING (A) or SET-PHRASE (A2) answer, end on the plain meaning and add nothing else — no story plot, no character actions.
